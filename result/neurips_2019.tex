...
Given an input $x$ with an output $y$, the prediction is $\hat{y}$. 

The formulation of squared loss is:
$$squared\_loss = \sum{(y - \hat{y})^2}$$

Given the The formulation of huber loss is:
$$huber\_loss = \left\{\begin{matrix}
0.5 *  (y - \hat{y})          & if |(y - \hat{y})| <= d\\ 
d*|(y - \hat{y})| - 0.5 * d^2 & if |(y - \hat{y})| >  d
\end{matrix}\right.$$

The profermance of two loss function shows in Figure\ref{loss_fuc}. Huber loss works better in this case, because Squared loss tends to be dominated by outliers.

\subsection{Hybrid Loss Function}

I implement the following hybrid loss function and the profermance shows in Figure \ref{hybrid_loss}
$$hybrid\_loss = (y - \hat{y})^2 + |y - \hat{y}|$$
